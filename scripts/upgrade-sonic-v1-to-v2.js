...
async function main() {
    ...
    const PROXY_ADDRESS = process.env.SONIC_NODEBOOSTER_PROXY_ADDRESS;
    
    if (!PROXY_ADDRESS) {
        console.error("❌ Please set SONIC_NODEBOOSTER_PROXY_ADDRESS environment variable");
        console.log("Example: export SONIC_NODEBOOSTER_PROXY_ADDRESS=0x1234...");
        process.exit(1);
    }

    console.log("📍 Existing SonicNodeBooster proxy address:", PROXY_ADDRESS);
    ...
    const NodeBoosterV2 = await ethers.getContractFactory("SonicNodeBoosterV2");
    ...
    try {
        ...
        // Verify the upgrade
        console.log("\n🔍 Verifying upgrade...");
        const v2Contract = await ethers.getContractAt("SonicNodeBoosterV2", PROXY_ADDRESS);
