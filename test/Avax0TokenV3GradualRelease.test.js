...
describe("Avax0TokenV3 - Gradual Release Functionality", function () {
    ...
    describe("Gradual Release Configuration", function () {
        ...
        it("Should allow disabling gradual release", async function () {
            await token.setDefaultGradualReleaseConfig(0, 0, false);
    ...
    describe("Time Lock Creation with Gradual Release", function () {
        it("Should create time lock with default gradual release config", async function () {
            const lockAmount = ethers.parseEther("20000");
            const currentBlock = await ethers.provider.getBlock('latest');
            const releaseTime = currentBlock.timestamp + WEEK;
            
            await expect(
                token.connect(owner).createTimeLock(user1.address, lockAmount, releaseTime)
            ...
            expect(lock.gradualConfig.duration).to.equal(MONTH);
            expect(lock.gradualConfig.interval).to.equal(DAY);
            expect(lock.gradualConfig.enabled).to.be.true;
        });
    ...
    describe("Backward Compatibility", function () {
        it("Should maintain V2 function compatibility", async function () {
            // Test old function names work
            const balance = await token.connect(owner).getAvailableBalanceWithAutoRelease(user1.address);
            expect(balance).to.equal(ethers.parseEther("100000"));
            expect(await token.totalLockedAmount(user1.address)).to.equal(0);
        });
        
        it("Should work with V2-style lock creation", async function () {
            const lockAmount = ethers.parseEther("25000");
            const currentBlock = await ethers.provider.getBlock('latest');
            const releaseTime = currentBlock.timestamp + WEEK;
            
            // Use V2-style function (without gradual config)
            await token.connect(owner)["createTimeLock(address,uint256,uint256)"](user1.address, lockAmount, releaseTime);
            
            // Should use default gradual config
            const lock = await token.getTimeLock(user1.address, 0);
            expect(lock.gradualConfig.enabled).to.be.true;
            expect(lock.gradualConfig.duration).to.equal(MONTH);
            expect(lock.gradualConfig.interval).to.equal(DAY);
        ...
        it("Should support old timeLocks array access", async function () {
            ...
            // Test old array access method
            const [amount, time, released] = await token.timeLocks(user1.address, 0);
            expect(amount).to.equal(lockAmount);
            expect(time).to.equal(releaseTime);
    ...
    describe("Edge Cases", function () {
        ...
        it("Should handle very small amounts with gradual release", async function () {
            const lockAmount = ethers.parseEther("0.001"); // Very small amount
            const currentBlock = await ethers.provider.getBlock('latest');
            const releaseTime = currentBlock.timestamp + 10;
            ...
            // Should still work with gradual release
            const status = await token.getGradualReleaseStatus(user1.address, 0);
            expect(status.availableNow).to.be.gt(0);
        });
        ...
        it("Should handle burn operations with gradual release", async function () {
            const lockAmount = ethers.parseEther("20000");
            const currentBlock = await ethers.provider.getBlock('latest');
            const releaseTime = currentBlock.timestamp + 10;
            ...
            // Burn should process gradual release and succeed
            ...
            // Check that gradual release was processed
            const status = await token.getGradualReleaseStatus(user1.address, 0);
            expect(status.totalReleased).to.be.gt(0);
        });
    });
});