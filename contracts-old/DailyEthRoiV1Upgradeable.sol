...
interface iDailyRoiOnline {
    ...
    function getAllDeposits(address _addr) external view returns (DailyEthRoiV1Upgradeable.Deposit[] memory);
...
contract DailyEthRoiV1Upgradeable is Initializable, ReentrancyGuardUpgradeable, ContextUpgradeable, OwnableUpgradeable {
    ...
    uint16[20] private ref_bonuses;
    uint256 DAILY_PROFIT = 50;
    uint256 private maxPayoutDays;
    ...
    uint256 private systemPool1Rate;
    uint256 private systemPool2Rate;
    uint256 private systemPool3Rate;
    uint256 private systemPool4Rate;
    uint256 private systemPool5Rate;
    uint256 private systemPoolTotalRate;
    ...
    function initialize(
        ...
        ref_bonuses = [80,30,20,10,10,10,10,10,5,5,5,5,5,5,5,1,1,1,1,1];
                
        defaultReferrer = msg.sender;  

        DAILY_PROFIT = 50; // 5% daily profit
    ...
    function withdraw() public nonReentrant notPaused notBlacklisted {
        ...
        for (uint256 i = 0; i < player.deposits.length; i++) {
            ...
            (,,uint256 maxEligibleTime,,uint256 payout,,,,) = _computePayoutForDeposit(msg.sender, i);
            if (payout == 0) continue; 


            totalPayout += (payout * 30) / 100;
            dep.remainingCapital -= (payout * 70) / 100;
    ...
    function computePayout(address _addr) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < player.deposits.length; i++) {
            (,,,,uint256 payout,,,,) = _computePayoutForDeposit(_addr, i);
    ...
    function getPlayersStructuresFromOldContract(address _oldContractAddress) external onlyOwner
    {        
        iDailyRoiOnline oldContract = iDailyRoiOnline(_oldContractAddress);

        
        for (uint256 i = 0; i < playersList.length; i++) {
            address playerAddress = playersList[i];
            uint256[20] memory oldStructure = oldContract.getAllStructures(playerAddress);
            
            require(oldStructure.length == 20, "Invalid structure length from old contract");

            delete players[playerAddress].structure; 
            
            for (uint8 j = 0; j < 20; j++) {
                players[playerAddress].structure[j] += oldStructure[j];
            }
        }
    }

    function getPlayersDepositsFromOldContract(address _oldContractAddress) external onlyOwner
    {        
        iDailyRoiOnline oldContract = iDailyRoiOnline(_oldContractAddress);
        
        for (uint256 i = 0; i < playersList.length; i++) {
            address playerAddress = playersList[i];
            Deposit[] memory oldDeposits = oldContract.getAllDeposits(playerAddress);
                        
            require(oldDeposits.length > 0, "No deposits found for this address");            

            delete players[playerAddress].deposits; 

            for (uint256 j = 0; j < oldDeposits.length; j++) {
                players[playerAddress].deposits.push(oldDeposits[j]);
            }
        }
    }

    function getPlayersWithdrawalsFromOldContract(address _oldContractAddress) external onlyOwner
    {        
        iDailyRoiOnline oldContract = iDailyRoiOnline(_oldContractAddress);

        for (uint256 i = 0; i < playersList.length; i++) {
            address playerAddress = playersList[i];
            (uint256[] memory amounts, uint40[] memory times) = oldContract.getAllWidthrawal(playerAddress);
            
            require(amounts.length == times.length, "Array lengths must match");
            require(amounts.length > 0, "Arrays cannot be empty");
            delete withdrawalHistory[playerAddress]; 

            for (uint256 j = 0; j < amounts.length; j++) {
                withdrawalHistory[playerAddress].push(Withdrawal({
                    amount: amounts[j],
                    time: times[j]
                }));
            }
